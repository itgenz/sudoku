...
const game = {
    ...
    lastTapTime: 0 // Thêm biến để theo dõi thời gian tap
};
...
function selectTile(row, col, tile) {
    ...
    // Kiểm tra double tap (trong vòng 300ms)
    const currentTime = new Date().getTime();
    const tapLength = currentTime - game.lastTapTime;
    game.lastTapTime = currentTime;

    if (tapLength < 300 && tapLength > 0) {
        // Double tap - bỏ highlight
        if (game.selectedTile === tile) {
            tile.classList.remove('selected-cell');
            clearHighlights();
            game.selectedTile = null;
            return;
        }
